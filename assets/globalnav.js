...
AC.GlobalNav.prototype.enhanceSearch = function() {
    ...
    if (this.globalSearchForm && this.globalSearch) {
        ...
        if (SearchShortcut.geoMap[searchCountry.toUpperCase()].directory) {
            var c = SearchShortcut.geoMap[searchCountry.toUpperCase()].directory
        } else {
            if (searchCountry != "us") {
                var c = "/" + searchCountry.replace(/_/, "")
            } else {
                c = ""
            }
        }
        var b = {
            global: "http://www.apple.com" + c + "/search/",
            ipad: "http://www.apple.com" + c + "/search/",
            iphone: "http://www.apple.com" + c + "/search/",
            ipoditunes: "http://www.apple.com" + c + "/search/",
            mac: "http://www.apple.com" + c + "/search/",
            store: "http://www.apple.com" + c + "/search/",
            support: "http://www.info.apple.com/searchredir.html"
        };
        var a = b[searchSection] || "http://www.apple.com/search/";
        this.globalSearchForm.setAttribute("action", a);
        ...
        SearchShortcut.loadXmlToDoc = function(d) {
            searchShortcut.loadXmlToDoc(d)
        }
    }
};
AC.GlobalNav.prototype.decorateSearchInput = function() {
    if (this.globalSearch) {
        ...
        c = this,
        a = true,
        f = document.getElementById("g-search");
        g = document.createDocumentFragment();
        this.globalSearch.setAttribute("autocomplete", "off");
        b = document.createElement("input");
        this.globalSearch.parentNode.replaceChild(b, this.globalSearch);
        e = document.createElement("div");
        e.className = "reset";
        resetEnd = function(h) {
            if (h.target == e && window.getComputedStyle(e, null)["opacity"] == "0") {
                e.style.display = "none"
            }
        };
        if (window.addEventListener) {
            e.addEventListener("transitionend", resetEnd, true);
            e.addEventListener("transitionEnd", resetEnd, true);
            e.addEventListener("oTransitionEnd", resetEnd, false);
            e.addEventListener("mozTransitionEnd", resetEnd, false);
            e.addEventListener("webkitTransitionEnd", resetEnd, false)
        }
        if (this.globalSearch.value.length == 0) {
            f.className += " empty"
        }
        g.appendChild(this.globalSearch);
        g.appendChild(e);
        d = function(h) {
            a = false;
            c.globalSearch.value = "";
            if (c.searchShortcut) {
                c.searchShortcut.hideResults()
            }
            window.setTimeout(function() {
                f.className += " empty";
                a = true
        ...
        AC.addEvent(e, "mousedown", d);
        AC.addEvent(this.globalSearch, "focus",
        function(h) {
            if (a) {
                e.style.display = "";
                window.setTimeout(function() {
                    c.globalHeader.className += " searchmode"
                },
                10)
            }
        });
        AC.addEvent(this.globalSearch, "blur",
        function(h) {
            if (a) {
                e.style.display = "";
                window.setTimeout(function() {
                    AC.removeClassName(c.globalHeader, "searchmode")
                },
                10)
            } else {
                a = true
        ...
        AC.addEvent(this.globalSearch, "keydown",
        function(h) {
            var i = h.keyCode;
            a = true;
            if (c.globalSearch.value.length >= 0) {
                e.style.display = "";
                window.setTimeout(function() {
                    AC.removeClassName(f, "empty")
                },
                10)
            } else {
                if (!f.className.match("empty")) {
                    f.className += " empty"
                }
            }
            if (h.keyCode === 27) {
...
var SearchShortcut = function(d, b) {
    this.searchWrapper = document.getElementById("globalsearch");
    this.searchForm = d;
    this.resultsPanel = document.getElementById("sp-results");
    this.searchInput = b;
    this.addSection();
    var c = (/applewebkit/i.test(navigator.userAgent) && /mobile/i.test(navigator.userAgent)) || /webos/i.test(navigator.userAgent) || /android/i.test(navigator.userAgent) || /blackberry/i.test(navigator.userAgent) || /windows ce/i.test(navigator.userAgent) || /opera mini/i.test(navigator.userAgent);
    if (c || (typeof(deactivateSearchShortcuts) !== "undefined" && deactivateSearchShortcuts)) {
        return
    }
    if (this.shouldVML()) {
        this.resultsPanel.className += " sp-results-vml"
    }
    this.addSpinner();
    ...
    AC.addEvent(document, "mousemove",
    function(e) {
        if (!a.resultsShowing) {
            return
        }
    ...
    AC.addEvent(this.searchInput, "keydown",
    function(e) {
        a.onKeyDown(e)
    });
    AC.addEvent(this.searchInput, "keyup",
    function(e) {
        a.onKeyUp(e)
    });
    AC.addEvent(this.searchInput, "blur",
    function(e) {
        a.onBlur(e)
    })
};
...
SearchShortcut.prototype.addSection = function() {
    var a = document.getElementById("search-section");
    if (!a) {
        a = document.createElement("input");
        a.id = "search-section";
        a.type = "hidden";
        a.name = "sec";
        a.value = window.searchSection;
        this.searchForm.appendChild(a)
    } else {
        if (a) {
            a.value = window.searchSection
        }
...
SearchShortcut.prototype.onKeyDown = function(a) {
    var b = typeof(event) != "undefined" ? event.keyCode: a.keyCode;
    if (!a) {
        a = event
    }
    if (b == 13 && !a.altKey) {
...
SearchShortcut.prototype.onKeyUp = function(a) {
    var d = typeof(event) != "undefined" ? event.keyCode: a.keyCode;
    if (!a) {
        a = event
    }
    if (d == 40 && this.results) {
        try {
            a.preventDefault();
            a.stopPropagation()
        } catch(c) {}
        if (this.selected && this.results[this.selected.index + 1]) {
            ...
            if (!this.selected && this.results[0]) {
                this.selected = this.results[0].select()
            }
        }
    } else {
        if (d == 38 && this.results) {
            try {
                a.preventDefault();
                a.stopPropagation()
            } catch(c) {}
            ...
            this.selected = false;
            var b = this.searchInput.value;
            b = b.replace(/[%\^\?\!\*\/<>\$]/ig, "").replace(/^\s+/g, "").replace(/\s+$/g, "");
            if (b.length > this.minimumCharactersForSearch) {
                this.searchText = b;
...
SearchShortcut.prototype.onMouseMove = function(a) {
    a = a || window.event;
...
SearchShortcut.prototype.isOverResults = function(a) {
    if (!this.mouseEventTarget) {
        return false
    }
    while ((this.mouseEventTarget.id !== "sp-results") && this.mouseEventTarget.parentNode) {
        this.mouseEventTarget = this.mouseEventTarget.parentNode
    }
    return (this.mouseEventTarget.id === "sp-results")
};
SearchShortcut.prototype.onBlur = function(a) {
    if (this.isOverResults()) {
...
SearchShortcut.prototype.fullSearchUrl = function() {
    var a = this.searchForm.getAttribute("action");
    return a
};
SearchShortcut.prototype.getQueryParameters = function(c) {
    var b = this.searchForm.elements,
    a,
    d,
    e = this.searchForm.getAttribute("action");
    this._formValues = [];
    for (a = b.length - 1; a >= 0; a--) {
        var d = b[a];
        if (d.name !== "q" && e.indexOf(d.name) === -1) {
            this._formValues.push(d.name + "=" + d.value);
            this._formValues[d.name] = d.name
        }
    }
    var f = "?q=" + encodeURIComponent(c);
...
SearchShortcut.prototype.search = function(c) {
    var a = this.baseUrl + this.getQueryParameters(c);
    this.showSpinner();
    a += "&transport=js";
...
SearchShortcut.prototype.loadXmlToDoc = function(b) {
    var a;
    ...
    this.hideSpinner();
    this.term = a.getElementsByTagName("term")[0].firstChild.nodeValue;
    this.xml = a.getElementsByTagName("search_results")[0];
    this.parseResults(this.xml);
    if (this.results) {
        this.renderResults()
    }
};
SearchShortcut.prototype.parseResults = function(d) {
    var c = d.getElementsByTagName("error");
    if (c.length > 0) {
        this.hideResults();
        return
    } else {
        var f = d.getElementsByTagName("match");
        ...
        for (var e = 0; e < f.length; e++) {
            ...
            var b = {
                ...
                desc: a.getAttribute("copy"),
                category: a.getAttribute("category"),
                priority: a.getAttribute("priority"),
                image: a.getAttribute("image")
            };
            b.url = decodeURIComponent(b.url);
            this.results.push(b)
        }
    }
};
SearchShortcut.prototype.renderResults = function() {
    ...
    if (this.shouldVML()) {
        ...
        roundrect.appendChild(e);
        g.appendChild(roundrect)
    } else {
        g.appendChild(e)
    }
    if (b === 0) {
        this.results[0] = {
            ...
            url: this.fullSearchUrl() + this.getQueryParameters(this.term)
        };
        c.className = "noresults"
    } else {
        d.innerHTML = this.quickLinks;
        e.appendChild(d)
    }
    var b = this.results.length;
    for (var a = 0; a < b; a++) {
        if (a > 5) {
            delete this.results[a]
        } else {
            this.results[a] = new SearchShortcut.result(a, this.results[a]);
            c.appendChild(this.results[a].element)
        }
    }
    e.appendChild(c);
    this.resultsPanel.appendChild(g);
    if (this.shouldVML()) {
...
SearchShortcut.prototype.hideResults = function(a, b) {
    this.selected = false;
...
SearchShortcut.prototype.track = function(d, a) {
    ...
    if (typeof(searchCountry) != "undefined" && searchCountry && searchCountry != "US") {
        b = SearchShortcut.geoMap[searchCountry.toUpperCase()].code
    }
    if (b) {
        c = "apple" + b + "global";
        e = "apple" + b + "search"
    ...
    s.eVar2 = "WWW-sc: " + d.toLowerCase();
    s.prop7 = "WWW-sc: " + d.toLowerCase();
    s.eVar4 = a;
    s.prop10 = a;
    s.tl(this, "o", "Shortcut Search")
};
SearchShortcut.prototype.fullSearch = function(a) {
    var b = this.fullSearchUrl() + this.getQueryParameters(this.searchText);
    document.location = b.replace("q=undefined&", "")
};
SearchShortcut.prototype.go = function(a) {
    this.track(this.searchText, a);
    document.location = a
};
SearchShortcut.prototype.shouldHideQuicktimeMovies = function() {
    ...
    d = /firefox/i.test(c),
    a = /chrome/i.test(c),
    e = (/applewebkit/i.test(c) && !a);
    win = /windows/i.test(c),
    mac = /mac/i.test(c);
    if (mac && (e || a)) {
        return false
    }
...
SearchShortcut.prototype.hideAllQuicktimeMovies = function() {
    if (this.shouldHideQuicktimeMovies()) {
        if (typeof(AC) != "undefined" && typeof(AC.Quicktime) != "undefined" && typeof(AC.Quicktime.controllers) != "undefined") {
            ...
            var a = AC.Quicktime.controllers;
            var b = {
                width: 328,
                height: 448
            };
            var r = k(this.resultsPanel);
            var q = r[0] - 328;
            var p = r[1];
            var o = m + b.width;
            var n = l + b.height;
            for (var f = a.length - 1; f >= 0; f--) {
                var d = a[f].movie;
                var j = Element.getDimensions(d);
                var g = k(d);
                var m = g[0];
                var l = g[1];
                if (c(m, l, j.width, j.height, q, p, b.width, b.height)) {
                    this.pausedControllers.push(a[f]);
                    a[f].Stop();
                    a[f].movie.style.visibility = "hidden"
                }
            }
        } else {
            this.qtm = document.getElementsByTagName("object");
            for (var f = 0; f < this.qtm.length; f++) {
                if (typeof(this.qtm[f].Stop) != "undefined") {
                    this.qtm[f].Stop()
                }
                try {
                    if (typeof(this.qtm[f].getElementsByTagName("embed")[0].Stop) != "undefined") {
                        this.qtm[f].getElementsByTagName("embed")[0].Stop()
                    }
                } catch(h) {}
                this.qtm[f].style.visibility = "hidden"
            }
        }
    }
};
SearchShortcut.prototype.showAllQuicktimeMovies = function() {
    if (typeof(AC) != "undefined" && typeof(AC.Quicktime) != "undefined" && typeof(AC.Quicktime.controllers) != "undefined") {
        for (var a = this.pausedControllers.length - 1;
        a >= 0; a--) {
            this.pausedControllers[a].movie.style.visibility = "visible";
            if (navigator.userAgent.match(/Firefox/i)) {
                this.pausedControllers[a].movie.style.zIndex = "100";
                setTimeout(this.pausedControllers[a].Play.bind(this.pausedControllers[a]), 100)
            } else {
                this.pausedControllers[a].Play()
            }
        }
        this.pausedControllers = []
    } else {
        if (this.qtm) {
            for (var a = 0; a < this.qtm.length; a++) {
                this.qtm[a].style.visibility = "visible";
                if (typeof(this.qtm[a].Play) != "undefined") {
                    this.qtm[a].Play()
                }
                try {
                    if (typeof(this.qtm[a].getElementsByTagName("embed")[0].Play) != "undefined") {
                        this.qtm[a].getElementsByTagName("embed")[0].Play()
                    }
                } catch(b) {}
            }
        }
...
SearchShortcut.descriptionCharacters = 105;
SearchShortcut.titleCharacters = 39;
SearchShortcut.result = function(a, b) {
    this.index = a;
    this.data = b;
    this.data.truncated = {};
    if (this.data.desc) {
        this.data.truncated.desc = unescape(this.data.desc);
        if (this.data.truncated.desc.length > SearchShortcut.descriptionCharacters) {
            this.data.truncated.desc = this.data.truncated.desc.substring(0, this.data.truncated.desc.lastIndexOf(" ", this.descriptionCharacters - 11)) + "&hellip;"
        }
    }
    if (this.data.title) {
        this.data.truncated.title = unescape(this.data.title);
        if (this.data.truncated.title.length > SearchShortcut.titleCharacters) {
            this.data.truncated.title = this.data.truncated.title.substring(0, this.data.truncated.title.lastIndexOf(" ", 30)) + "&hellip;"
        }
    }
    this.render()
};
SearchShortcut.result.prototype.render = function() {
    ...
    if (this.data.url) {
        ...
        d.href = decodeURIComponent(this.data.url);
        AC.addEvent(d, "mousedown",
        function(h) {
            if (searchShortcut) {
                searchShortcut.go(a.data.url)
            }
        });
        e.appendChild(d)
    }
    if (this.data.image) {
        b = new Image();
        b.src = this.data.image;
        b.alt = this.data.title;
        d.appendChild(b);
        if (/MSIE (5\.5|6\.)/.test(navigator.userAgent)) {
            b.src = "/global/elements/blank.gif";
    ...
    if (this.data.truncated.title) {
        ...
        f.innerHTML = this.data.truncated.title;
        d.appendChild(f)
    }
    if (this.data.truncated.desc) {
        c = document.createElement("p");
        c.innerHTML = this.data.truncated.desc;
        d.appendChild(c)
    }
    this.element = e
};
...
SearchShortcut.geoMap = {
    ...
    BE_FR: {
        code: "bf",
        noResults: "Pas de résultat. Essayez une recherche apple.com",
        viewAll: "Afficher tous les résultats",
    ...
    BR: {
        code: "br",
        noResults: "Não encontrado. Tente a busca em apple.com",
        viewAll: "Ver todos os resultados da busca",
        shortcuts: "Links rapidos",
        searchText: "Buscar"
    },
    CA_EN: {
        code: "ca",
    ...
    DK: {
        code: "dk",
        noResults: "Ingen genvej fundet. Prøv at søge på hele apple.com.",
        viewAll: "Vis alle søgeresultater",
        shortcuts: "Hurtige henvisninger",
        searchText: "Søg"
    },
    ES: {
        code: "es",
        noResults: "Ningún atajo. Búsqueda completa en apple.com",
        viewAll: "Ver todos los resultados de búsqueda",
        shortcuts: "Enlaces rápidos",
        searchText: "Buscar"
    },
    FI: {
        code: "fi",
        noResults: "Ei oikotietä. Etsi koko apple.com.",
    ...
    JP: {
        code: "jp",
        noResults: "ショートカットは見つかりませんでした。検索はこちら。",
        viewAll: "すべての検索結果を見る",
        shortcuts: "クイックリンク",
        searchText: "Search"
    },
    KR: {
        code: "kr",
        noResults: "일치하는 검색결과가 없습니다. 다시 검색하기.",
        shortcuts: "빠른 연결",
        viewAll: "검색 결과 전체 보기."
    },
    LA: {
        code: "la",
        noResults: "No se encontraron resultados. Intenta en apple.com.",
        viewAll: "Ver todos los resultados de la búsqueda",
        shortcuts: "Enlaces rápidos",
        searchText: "Buscar"
    },
    LAE: {
    ...
    NO: {
        code: "no",
        noResults: "Fant ingen snarvei. Søk på hele apple.com.",
        viewAll: "Vis alle søkeresultater",
        shortcuts: "Hurtigkoblinger",
        searchText: "Søk"
    },
    NZ: {
        code: "nz"
    ...
    PL: {
        code: "pl",
        noResults: "Fraza nie została odnaleziona. Użyj apple.com.",
        viewAll: "Przeglądaj wszystkie wyniki",
        shortcuts: "Podręczne łącza",
        searchText: "Szukaj"
    },
    PT: {
        code: "pt",
        ...
        shortcuts: "Ligações rápidas",
        searchText: "Procurar"
    },
    RU: {
        code: "ru",
        noResults: "Ссылок нет. Попробуйте расширенный поиск.",
    ...
    SE: {
        code: "se",
        noResults: "Ingen genväg hittad. Sök i hela apple.com.",
        viewAll: "Visa alla sökresultat",
        shortcuts: "Snabblänkar",
        searchText: "Sök"
    },
    SG: {
        code: "sg"
